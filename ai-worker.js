 ...
 * type: 'module' Worker として起動すること (main.js: new Worker(url, { type: 'module' }))
 *
 * 修正点:
 *  - wllama@2.3.0 の正しいESMパス: esm/index.js (dist/wllama.js は存在しない)
 *  - WASMパスも esm/single-thread/ esm/multi-thread/ に修正
 *  - Transformers.js も正しいESMパスに修正
 */
...
async function loadWllama() {
  ...
  console.log('[Ankel Worker] Wllama ESM import 開始...');
  try {
    // 正しいエントリポイント: esm/index.js
    const mod = await import('https://cdn.jsdelivr.net/npm/@wllama/wllama@2.3.0/esm/index.js');
    WllamaClass = mod.Wllama;
    if (typeof WllamaClass !== 'function') {
      throw new Error('Wllama クラスが見つかりません。keys: ' + Object.keys(mod).join(', '));
    }
...
async function loadTransformers() {
  ...
  console.log('[Ankel Worker] Transformers.js ESM import 開始...');
  try {
    // @xenova/transformers v2 の正しいESMエントリポイント
...
async function handleGenerate({ messages, context }, id) {
  ...
  await wllamaInst.createCompletion(prompt, {
    nPredict: 800, temperature: 0.7, topP: 0.9,
