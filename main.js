...
const APP_VERSION    = '2025-05-23-r7';  // ← デプロイ時に変更
const WORKER_VERSION = '20250523f';
...
async function sendMessage() {
  ...
  try {
    ...
    await wllamaInst.createCompletion(prompt, {
      ...
      tfsZ:        1.0,
      typicalP:    1.0,
      repeatPenalty:      1.18,   // 繰り返しペナルティ
      repeatLastN:        128,    // 直近N tokenに対してペナルティ
      penalizeNl:         false,  // 改行はペナルティ対象外
      // 停止トークン（モデルのEOS・ターンセパレータ）
      stopTokens: [2, 128001, 128009], // EOS, <|eot_id|>, <|end_of_text|>
      onNewToken: (_t, _p, piece, { abortSignal } = {}) => {
        // ユーザーキャンセル
        if (signal.aborted) return true; // true = 停止シグナル

        // 繰り返し検出（直近100文字に同じ20文字以上のフレーズが2回以上）
        recentTokens += piece;
        if (recentTokens.length > 200) recentTokens = recentTokens.slice(-200);
        if (!abortedByRepeat && recentTokens.length >= 60) {
          const half = Math.floor(recentTokens.length / 2);
          const latter = recentTokens.slice(-half);
          // 後半部が前半にも含まれているなら繰り返し
          const former = recentTokens.slice(0, -half);
          if (latter.length >= 20 && former.includes(latter.slice(0, 20))) {
            console.warn('[Ankel] 繰り返しを検出。生成を停止します');
            abortedByRepeat = true;
            return true; // 停止
          }
        }

        bubbleBody.textContent += piece;
    ...
    // ユーザーキャンセルは正常終了扱い
    if (!signal.aborted) {
      bubbleBody.textContent += (bubbleBody.textContent ? '' : '') ;
