...
const APP_VERSION    = '2025-05-24-r11';
...
const MODEL_CONTEXT_WINDOW = {
  ...
  'Llama-3.2-3B-Instruct-q4f32_1-MLC': 4096,
};
...
/**
 * クエリをScrapbox命名規則（yyyy-タグ-説明）に沿って単語単位で完全一致スキャン
 * 例: "2024年度の広報活動" → tokens["2024","広報"] で
 *     タイトルセグメント "2024", "広報" が完全一致するページを優先ヒット
 */
function titleSegmentExactMatch(chunk, queryTokens) {
  const segs = (chunk.allSegments || []).map(s => s.toLowerCase());
  let hits = 0;
  for (const token of queryTokens) {
    if (segs.includes(token)) hits++;
    // 部分一致（セグメントがトークンを含む）
    if (segs.some(s => s.includes(token) && s !== token)) hits += 0.5;
  }
  return hits;
}
...
async function searchKeyword(query, topK = 10) {
  ...
  const scored = chunks.map(chunk => {
    ...
    for (const token of queryTokens) {
      if (!token) continue;
      // タイトル完全一致（最優先）
      if (titleL === token) score += 40;
      // タイトル部分一致
      if (titleL.includes(token)) score += 16;
      // セグメント完全一致（命名規則を活用した精密マッチ）
      const segExact = (chunk.allSegments || []).some(s => s.toLowerCase() === token);
      if (segExact) score += 14;
      ...
      if (bodyCount > 0) score += Math.min(bodyCount, 6) * 2;
    }

    // ── 2. 命名規則マッチボーナス（複数セグメントが同時一致するほど高得点）──
    const segMatchCount = titleSegmentExactMatch(chunk, queryTokens);
    if (segMatchCount >= 2) score += segMatchCount * 12; // 2語以上同時セグメント一致
    else if (segMatchCount >= 1) score += segMatchCount * 6;

    // ── 3. 年度一致: 乗数ブースト ──
    let yearMultiplier = 1.0;
    queryMeta.years.forEach(year => {
      if (chunk.year === year) {
        yearMultiplier = Math.max(yearMultiplier, 1.6);
      } else if (Math.abs((chunk.year || 0) - year) === 1) {
        yearMultiplier = Math.max(yearMultiplier, 1.15);
      }
    });
