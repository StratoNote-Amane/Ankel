...
const WORKER_VERSION = '20250523c';
...
async function loadModel(key) {
  ...
  try {
    // Wllama はメインスレッドで直接呼ぶ（Worker内ではdocumentが使えないため）
    ...
    // QuotaExceededError の場合 OPFS をクリアして再試行を促す
    if (err.message && err.message.includes('QuotaExceeded')) {
      showToast('ストレージ不足。OPFSをクリアします...');
      await clearOPFS();
      showToast('クリア完了。もう一度モデルを選択してください。');
    }
  } finally {
