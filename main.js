...
async function handleJSONFile(file) {
  if (!file.name.endsWith('.json')) {
    ...
    console.warn('[Ankel] ファイル拒否: .json 以外のファイルが選択された', file.name);
    return;
  }

  console.log('[Ankel] ファイル受付:', file.name, `(${(file.size / 1024).toFixed(1)} KB)`);
  setImportStatus('JSONを解析中...', '');
  setImportProgress(0, null); // バー表示開始

  try {
    console.log('[Ankel] file.text() 開始...');
    const raw  = await file.text();
    console.log('[Ankel] file.text() 完了。文字数:', raw.length);

    const json = JSON.parse(raw);
    console.log('[Ankel] JSON.parse 完了。pages数:', json.pages?.length ?? 'pagesキーなし');

    if (!Array.isArray(json.pages)) {
      setImportStatus('Scrapbox JSONの形式が正しくありません', 'error');
      console.error('[Ankel] json.pages が配列ではありません:', typeof json.pages);
      hideImportProgress();
      return;
    }

    console.log('[Ankel] DB書き込み開始...');
    const done = await importScrapboxJSON(json, (d, t) => {
      const pct = Math.round((d / t) * 100);
      setImportStatus(`${d} / ${t} ページを保存中...`, '');
      setImportProgress(pct, `${pct}%`);
      console.log(`[Ankel] 保存進捗: ${d}/${t} (${pct}%)`);
    });

    console.log(`[Ankel] DB書き込み完了: ${done}ページ`);
    setImportProgress(100, '100%');
...
/** インポート進捗バーを更新する。pct=0 で表示開始、null で非表示 */
...
async function launchApp() {
  console.log('[Ankel] launchApp: チャット画面を起動します');
...
async function startBackgroundEmbedding() {
  ...
  if (doneMeta && doneMeta.value === true) {
    embeddingReady = true;
    console.log('[Ankel] Embedding済み。スキップします');
    return;
  }
  ...
  try {
    await embedAllPagesInBackground((done, total) => {
      ...
      if (pct !== lastPct) {
        ...
        if (pct % 10 === 0) {
          console.log(`[Ankel] Embedding進捗: ${done}/${total} (${pct}%)`);
        }
      }
    });
    embeddingReady = true;
    console.log('[Ankel] Embedding完了。意味検索が有効になりました');
...
async function loadModel(key) {
  ...
  try {
    await sendWorker('LOAD_MODEL', { modelKey: key }, ({ status, progress }) => {
      ...
      if (progress % 10 === 0) {
        console.log(`[Ankel] モデルロード進捗: ${progress}% — ${status}`);
      }
    });
...
async function retrieveContext(query) {
  console.log('[Ankel] retrieveContext:', query.slice(0, 40), '| embeddingReady:', embeddingReady);
  ...
  const kwRes = await searchKeyword(query, 5);
